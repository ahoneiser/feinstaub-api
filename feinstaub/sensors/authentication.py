...
class NodeUidAuthentication(authentication.BaseAuthentication):
    def authenticate(self, request):
        node_uid = (
            request.META.get("HTTP_X_SENSOR")
            or request.META.get("HTTP_SENSOR")
            or request.META.get("HTTP_NODE")
        )
        if not node_uid:
            return None
