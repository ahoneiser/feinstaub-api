...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for dt, sensor in product(
            self._dates(start_date, end_date), Sensor.objects.all()
        ):
            ...
            qs = (
                SensorData.objects.filter(sensor=sensor)
                .filter(timestamp__date=dt)
                .order_by("timestamp")
            )
            if options.get("no_excludes"):
                qs = qs.exclude(location_id=11).exclude(location__indoor=True)
            if not qs.exists():
                continue
