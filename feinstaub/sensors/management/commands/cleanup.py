...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        SensorData.objects.annotate(Count("sensordatavalues")).filter(
            sensordatavalues__count=0
        ).filter(created__lt=datetime.date.today()).delete()

        # find all ppd42ns with wrong values and delete them. fixing is way to complicated
        SensorData.objects.filter(sensor_id__in=[34, 39, 60]).filter(
            sensordatavalues__value_type="temperature"
        ).delete()
        SensorData.objects.filter(sensor_id__in=[34, 39, 60]).filter(
            sensordatavalues__value_type="humidity"
        ).delete()
