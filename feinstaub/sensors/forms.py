...
class AddSensordeviceForm(forms.Form):
    ...
    location_latitude = forms.DecimalField(
        label="Latitude (Breite, ~48)",
        min_value=-90,
        max_value=90,
        decimal_places=10,
    )
    location_longitude = forms.DecimalField(
        label="Longitude (Länge, 9)",
        min_value=-180,
        max_value=180,
        decimal_places=10,
    )
    ...
    # device info
    device_initials = forms.CharField(
        label="Device initials (label to write on device)"
    )
    device_uid = forms.CharField(
        label="Device UID (esp8266-<chipid>)", initial="esp8266-"
    )
    ...
    sensor1_type = forms.ModelChoiceField(
        queryset=SensorType.objects.all(),
        to_field_name="name",
        initial="SDS011",
    )
    sensor1_pin = forms.DecimalField(
        label="PIN", min_value=0, max_value=8, decimal_places=0, initial=1
    )
    ...
    sensor2_type = forms.ModelChoiceField(
        queryset=SensorType.objects.all(),
        to_field_name="name",
        initial="DHT22",
    ...
    sensor2_pin = forms.DecimalField(
        label="PIN", min_value=0, max_value=8, decimal_places=0, initial=7
    )
